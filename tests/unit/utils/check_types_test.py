...
def test_check_field_type_with_dict_enum_incorrect():
    result, error = check_field_type(
        "status", "archived", {"enum": ["active", "inactive", "pending"]}
    )
    assert result is False
    assert error == (
        "Parameter status must be one of ['active', 'inactive', 'pending'], "
        "but got archived"
    )
...
def test_check_field_type_with_list_enum_correct():
    result, error = check_field_type(
        "roles", ["admin", "user"], {"enum": ["admin", "user", "guest"]}
    )
    assert result is True
    assert error is None
